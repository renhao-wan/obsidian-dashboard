...
import type { DashboardData, DashboardCard, LibraryConfig } from './types';
...
/**
 * Create the base (sync-only) callbacks shared between the full view and the
 * lightweight section-refresh path. UI-mutation callbacks (modals, scroll, etc.)
 * are added on top by `createCallbacks`.
 */
export function createBaseCallbacks(sync: SyncEngine) {
	return {
